...
import { shopReview } from "../../data/reviewsData";
...
const UserReviews = () => {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (rating >= i) {
        stars.push(
          <span key={i} className="userreview__star">
            <svg
              width="18"
              height="17"
              viewBox="0 0 18 17"
  ...
  return (
    <div className="userreview">
      <div className="userreview__top">
        <h1 className="userreview__heading head-48">OUR HAPPY CUSTOMERS</h1>
        <div className="userreview__arrows">
          <button className="userreview__arrow" onClick={handleScrollLeft}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <button className="userreview__arrow" onClick={handleScrollRight}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
      ...
      <div className="userreview__reviews" ref={sliderRef}>
        <div className="userreview__track" >
          {shopReview.map((review, item) => (
            <div key={item} className="userreview__review-card">
              <div className="userreview__stars">
                {renderStars(review.rating)}
              </div>
              <div className="userreview__account">
                <h2 className="userreview__name">{review.name}</h2>
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
              ...
              <p className="userreview__comment p-16">{`"${review.comment}"`}</p>
            </div>
          ))}
        </div>
        {/*May to set carousel, but without scroll only*/}
        {/*<div className="userreview__track">
          {shopReview.map((review, item) => (
            <div key={`clone-${item}`} className="userreview__review-card" aria-hidden>
              <div className="userreview__stars">
                {renderStars(review.rating)}
              </div>
              <div className="userreview__account">
